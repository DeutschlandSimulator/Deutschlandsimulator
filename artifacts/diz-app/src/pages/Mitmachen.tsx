    ...
    <Layout>
      <div className="flex-1 max-w-3xl mx-auto px-4 md:px-6 py-10 md:py-14 w-full flex flex-col gap-8">
        ...
        <section className="bg-[#1a2b3c] border border-[#1e3048] rounded-lg px-5 py-5">
          ...
          {/* Dimension 1: Wer? */}
          <p className="text-[9px] font-semibold uppercase tracking-widest text-[#8faabb] mb-1">Wer hat geprüft?</p>
          <div className="flex rounded-full overflow-hidden h-2 mb-2">
            <div className="bg-[#4caf82]" style={{ width: `${MENSCH_GEPRUEFT.length / ANNAHMEN.length * 100}%` }} title="Menschlich" />
            <div className="bg-[#60a5fa]" style={{ width: `${KI_GEPRUEFT.length / ANNAHMEN.length * 100}%` }}    title="KI" />
          </div>
          ...
          <p className="text-[9px] font-semibold uppercase tracking-widest text-[#8faabb] mb-1">Wie vollständig?</p>
          <div className="flex rounded-full overflow-hidden h-2 mb-2">
            <div className="bg-[#4caf82]" style={{ width: `${VOLLSTAENDIG.length / ANNAHMEN.length * 100}%` }}      title="Vollständig" />
            <div className="bg-[#f5a623]" style={{ width: `${TEILWEISE.length / ANNAHMEN.length * 100}%` }}         title="Teilweise" />
            <div className="bg-[#e05c5c]" style={{ width: `${NICHT_VERIFIZIERT.length / ANNAHMEN.length * 100}%` }} title="Nicht" />
          </div>
          <div className="grid grid-cols-3 gap-2 mb-4">
            {[
              { icon: "✅", label: "Vollständig",       count: VOLLSTAENDIG.length,      col: "#4caf82", bg: "bg-[#4caf82]/10 border-[#4caf82]/20" },
              { icon: "⚠️", label: "Teilweise belegt",  count: TEILWEISE.length,         col: "#f5a623", bg: "bg-[#f5a623]/10 border-[#f5a623]/20" },
              { icon: "❌", label: "Keine Quelle",      count: NICHT_VERIFIZIERT.length, col: "#e05c5c", bg: "bg-[#e05c5c]/10 border-[#e05c5c]/20" },
            ].map((s) => (
              <div key={s.label} className={`rounded border ${s.bg} px-3 py-2 text-center`}>
                <div className="text-lg font-bold" style={{ color: s.col }}>{s.count}</div>
                <div className="text-[9px] text-[#8faabb] leading-tight">{s.icon} {s.label}</div>
          ...
          <p className="text-[10px] text-[#f5a623] bg-[#f5a623]/5 border border-[#f5a623]/20 rounded px-3 py-2">
            <span className="font-semibold">Hinweis:</span> Aktuell gibt es {MENSCH_GEPRUEFT.length === 0 ? "noch keine" : MENSCH_GEPRUEFT.length} menschlich verifizierten Annahmen. Alle {KI_GEPRUEFT.length} Einträge wurden von der KI recherchiert und warten auf unabhängige Bestätigung.
          </p>
        </section>
